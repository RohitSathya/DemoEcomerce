...
export default function ProductInfo({data}) {
  ...
  async function test(){
    ...
    const response=await axios.post(link+'/product/cart',{name:data.name,category:data.category,price:data.price,image:data.image,uid:parse._id})
    const {message}=response.data
    if(message=='f'){
      alert('Product already have been added to cart')
    }
    else{
      const count=await axios.get(link+`/product/getcart/${parse._id}`)
      dispatch(fastcount())
